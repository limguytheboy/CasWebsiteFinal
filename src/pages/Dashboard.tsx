...
function safeFormatDate(value: Date | string | number | null | undefined, fmt = 'MMM d, yyyy') {
  const d =
        ...
        <aside className="lg:col-span-1">
          <div className="card-bakery">
            <div className="text-center">
              ...
              <h2 className="mt-3 font-bold text-foreground">{displayName}</h2>
            ...
            <button
              ...
              className="flex w-full items-center gap-3 rounded-xl px-4 py-3 text-muted-foreground hover:bg-destructive/10 hover:text-destructive"
            >
            ...
            {recentOrders.length === 0 ? (
              ...
              <div className="mt-4 space-y-4">
                {recentOrders.map(order => {
                  const paymentStatus =
                    order.paymentMethod === 'bca' ? 'Paid' : order.paymentMethod === 'cash' ? 'Unpaid' : '-'

                  const delivery = order.deliveryMethod ? String(order.deliveryMethod) : '-'

                  return (
                    <Link
                      key={order.id}
                      to={`/order/${order.id}`}
                      className="group flex items-center justify-between card-bakery"
                    >
                      <div className="space-y-1">
                        ...
                        <p className="text-sm text-muted-foreground">
                          {safeFormatDate(order.createdAt, 'MMM d, yyyy • h:mm a')} •{' '}
